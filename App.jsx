              ...
              <Ruled style={{ marginBottom: 8 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
                  <BookTitleInput
                    ...
                    onPick={(b) => setForm({ ...form, title: b.title, author: b.author, pages: String(b.pages || "") })}
                  />
...
function BookTitleInput({ value, onChange, onPick, placeholder }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const r = await fetch(
          `https://openlibrary.org/search.json?q=${encodeURIComponent(q)}&limit=8&fields=key,title,author_name,number_of_pages_median,cover_i,first_publish_year`
        );
        const d = await r.json();
        setResults(d.docs || []);
        setOpen(true);
